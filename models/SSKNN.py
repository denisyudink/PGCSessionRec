...
class SSKNN:
    ...
    def fit(self, train_data):
        """
        Fit the SSKNN model on the training data.
	...
	# Most of this section of the code is derived from: https://github.com/rn5l/session-rec.
        for session in recommendations:
            ...
            length = len(self.session_items) # Length of current session
            
            # Initialize variables
            current_pos = length 
            weight = 0
            
            # Iterate over current session reversed
            for i in reversed(self.session_items):
                if i in items:
                    weight = self.calc_pesos(current_pos, length) # calculate weight of the item if he is in Items of the nearest neighbors
                    break
                current_pos -= 1
            ...
            for item in items:
                ...
                score_updated = session[1] * weight # Update the score in the dictionary with the similarity of the session multiplied by the weight
