...
class SFSKNN:
    ...
    def fit(self, train_data):
        """
        Fit the SFSKNN model on the training data.
	...
	# Most of this section of the code is derived from: https://github.com/rn5l/session-rec.
        for _, row in train_data.iterrows():
            ...
            current_session_items.append(row[self.item_column])
    ...
    def is_subsequence(self, subsequence, sequence):
        """
        Verifiy if past session has the subsequence.

        Args:
            subsequence: Subsequence (Current Item -> Neighbour Item).
            sequence: Past session in order.

        Returns:
            True if has subsequence.
        """
        len_sub = len(subsequence)
        return any(subsequence == sequence[i:i + len_sub] for i in range(len(sequence) - len_sub + 1))
	...
	# Most of this section of the code is derived from: https://github.com/rn5l/session-rec.
        for session in recommendations:  
            ...
            for item in OrderedSet(items):
                sequence = list()
                sequence.append(self.session_items[-1]) # Append last item of the current session
                sequence.append(item) # Append item of the neigbour session
                
                if self.is_subsequence(sequence, items) == True:
