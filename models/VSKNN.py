...
class VSKNN:
    ...
    def __init__(self, k=500, sample_size=1000, list_length=20, session_column='SessionId', item_column='ItemId', timestamp_column='timestamp'):
        """
        Initialize the VSKNN class with hyperparameters and data column names.
    ...
    def fit(self, train_data):
        """
        Fit the VSKNN model on the training data.
    ...
    def predict_next(self, current_session, current_item, predict_next_items):
        ...
        # Calculate the weight of current session
        pesos = {}
        length = len(self.session_items)
        position = 1
        for item in self.session_items:
            pesos[item] = self.calc_pesos(position, length)
            position += 1
        
        # Calculate the similarity between the current session and filtered_dict
        similarities = list()
        for session, vector in filtered_sample_sessions.items():
            similarity = self.cosine(pesos, vector)
    ...
    def calc_pesos(self, pos, length):
        """
        ...
        """
        if (length - pos) >= 10:
            return 0.0 # Return 0.0 in case length - pos >= 10
        
        result = 1 - (0.1*(length-pos)) # Calculate weight
    ...
    def cosine(self, array1, array2):
        ...
        """
        # Get keys in common between the two arrays
        keys_set = OrderedSet(array1.keys()) & OrderedSet(array2)
        
        # Get the weights
        vector_list = [array1[key] for key in keys_set]
        
        # Sum the weights
        dot_product = sum(vector_list)
        
        # # Calculate the Euclidean distance of the current_session array
        dist_current = sum([value**value for value in array1.values()])

        # Calculate the Euclidean distance of the past_session array
        dist_past = len(set(array2))

        # Calculate the similarity
        similarity = dot_product / (sqrt(dist_past) * sqrt(dist_current)) 
