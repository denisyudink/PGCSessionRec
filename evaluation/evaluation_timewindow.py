...
def evaluate(df, item_column_name, session_column_name, timestamp_column, window_length_hours, model, limit):
    """
    ...
    for i in range(1, 31):  # Changed the upper limit to 'num_windows'
        print('iteration:', i)
        ...
        test_split = df[(df[timestamp_column] > df[timestamp_column].min() + pd.Timedelta(hours=i * window_length_hours)) &
                        (df[timestamp_column] <= df[timestamp_column].min() + pd.Timedelta(hours=(i + 1) * window_length_hours))]

        unique_sessions_test = test_split[session_column_name].unique()

        df_right_test = df[(df[timestamp_column] > df[timestamp_column].min() + pd.Timedelta(hours=(i + 1) * window_length_hours))]
        ...
        mean_mrr_window = np.mean(mrr_window)
        mean_hr_window = np.mean(hit_window)
        ...
        mean_mrr_iter = np.mean(mrr_scores)
        hit_rate_iter = np.mean(hit_rates)
